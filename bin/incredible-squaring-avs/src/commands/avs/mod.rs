...
impl<Ext: clap::Args + fmt::Debug + Send + Sync + 'static> AvsCommand<Ext> {
    ...
    pub async fn execute(self: Box<Self>) -> eyre::Result<()> {
        ...
        if let Some(config_path) = config_path {
            ...
            debug!("ecdsa key store path {:?}", ecdsa_keystore_path);
            debug!("ecdsa key password:{:?}", ecdsa_keystore_password);
            debug!("bls keystore path : {:?}", bls_keystore_path);
            debug!("bls keystore password : {:?}", bls_keystore_password);
            ...
            if config.chain_id().eq(&31337) {
                ...
                let erc20_mock_strategy_address_anvil =
                ...
                config
                    .set_rewards_coordinator_address(rewards_coordinator_address_anvil.to_string());
        ...
        if register_operator {
            let _ = register_operator_with_el_and_avs(
                ...
                config.rewards_coordinator_addr()?,
                ...
                U256::from(config.operator_1_token_amount()?),
            )
            .await;
            ...
            let _ = register_operator_with_el_and_avs(
                ...
                U256::from(config.operator_2_token_amount()?),
            )
            .await;
            if config.chain_id().eq(&31337) {
