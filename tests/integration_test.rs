...
async fn spawn_bin(name: &str) -> tokio::process::Child {
...
pub mod integration_test {
    ...
    async fn test_integration() {
        let mut children = vec![
            spawn_bin("incredible-squaring-aggregator").await,
            spawn_bin("incredible-squaring-challenger").await,
            spawn_bin("incredible-squaring-operator").await,
            spawn_bin("incredible-squaring-operator-2").await,
            spawn_bin("incredible-squaring-task-spammer").await,
        ];
        ...
        dbg!("Latest task num: {}", latest_task_num);
